...
//        if (function_exists('mb_substr')) return mb_substr($str, $offset, $length, 'utf-8'); #(PHP 4 >= 4.0.6, PHP 5)
        ...
        if ( $_str !== $str ) $_a = self::str_split( $_str = $str );
    ...
    public static function convert_case( $s, $mode )
    {
        if ( $mode === CASE_UPPER ){
            if ( !$s ) return $s;
            if ( preg_match( '/^[\x00-\x7e]*$/sSX', $s ) ) return strtoupper( $s ); #speed improve!
            ...
            if ( !$s ) return $s;
            if ( preg_match( '/^[\x00-\x7e]*$/sSX', $s ) ) return strtolower( $s ); #speed improve!
        ...
        $glue = null )
    {
        ...
        if ( $glue === '' ) $glue = "\r\n";
        if ( !is_array( $a = self::str_split( $string, $length ) ) ) return false;
...
//        if (function_exists('iconv_strpos')) return iconv_strpos($haystack, $needle, $offset, 'utf-8');
    ...
    public static function ucfirst( $s, $is_other_to_lowercase = true )
    {
        ...
        if ( !preg_match( '/^(.)(.*)$/usSX', $s, $m ) ) return false;
    ...
    public static function ucwords( $s )
    {
        ...
        foreach( $words as $k => $word ) $words[$k] = self::ucfirst( $word, $is_other_to_lowercase = true );
